...
export class Game {
    ...
    constructor(width: number, height: number, scale: number, speed: number = 100) {
        this.width = width;
        this.height = height;
    ...
    public get_empty_pos(min: number, max: number): Point {
        while (true) {
            ...
            if (!this.player.touch([pos])) {
                ...
                if (is_good) return pos;
            }
        }
    }

    public display_object(): void {
        // Holes
    ...
    public step(dir: DIRECTION): void {
    ...
    public partie(): boolean {
        this.display.draw(this);
        return false;
    }

    public create_rockhole(): void {
        this.rocks = [];
        this.holes = [];
